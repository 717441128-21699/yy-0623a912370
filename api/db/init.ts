...
export function initMockData() {
  const upsertUser = db.prepare(
    `INSERT INTO users (id, nickname, avatar, reputation, hosted_count, ghost_count, play_styles)
     VALUES (?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(id) DO UPDATE SET
       nickname = excluded.nickname,
       avatar = excluded.avatar,
       reputation = excluded.reputation,
       hosted_count = excluded.hosted_count,
  ...
  for (const user of users) {
    ...
    if (!existing) {
      upsertUser.run(user.id, user.nickname, user.avatar, user.reputation, user.hosted_count, user.ghost_count, user.play_styles);
      insertedUsers++;
    }
  }
  if (insertedUsers > 0) console.log(`Inserted ${insertedUsers} new users`);

  const upsertFleet = db.prepare(
    `INSERT INTO fleets (id, script_name, is_city_limited, script_type, atmosphere, city, district, location, start_time, total_players, current_players, host_id, roles, status, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  let insertedFleets = 0;
  for (const fleet of fleets) {
    const existing = db.prepare('SELECT id FROM fleets WHERE id = ?').get(fleet.id);
    if (!existing) {
      upsertFleet.run(
  ...
  if (insertedFleets > 0) console.log(`Inserted ${insertedFleets} new fleets`);

  const upsertReview = db.prepare(
    `INSERT INTO reviews (id, from_user_id, to_user_id, fleet_id, rating, comment)
     VALUES (?, ?, ?, ?, ?, ?)
     ON CONFLICT(id) DO NOTHING`
  );
  ...
  for (const review of reviews) {
    ...
    if (!existing) {
      upsertReview.run(review.id, review.from_user_id, review.to_user_id, review.fleet_id, review.rating, review.comment);
      insertedReviews++;
    }
  }
  if (insertedReviews > 0) console.log(`Inserted ${insertedReviews} new reviews`);

  const upsertSubscription = db.prepare(
    `INSERT INTO radar_subscriptions (id, user_id, script_name, city, notify_browser, notify_site)
     VALUES (?, ?, ?, ?, ?, ?)
     ON CONFLICT(id) DO NOTHING`
  );
  ...
  for (const sub of subscriptions) {
    const existing = db.prepare('SELECT id FROM radar_subscriptions WHERE id = ?').get(sub.id);
    if (!existing) {
      upsertSubscription.run(sub.id, sub.user_id, sub.script_name, sub.city, sub.notify_browser, sub.notify_site);
      insertedSubs++;
    }
  }
  if (insertedSubs > 0) console.log(`Inserted ${insertedSubs} new radar subscriptions`);
